...
-- ============================================================
-- ⚠️ 与 0004_fix_feedbacks_type_up.sql 内容等价（CHECK 允许值完全一致）。
--    0003 已应用于生产，保留此步仅为历史记录的完整性；
--    在空库上按 0001→0004 顺序执行两者都跑一遍也是安全的（已本地演练验证）。
--    今后若要再调整 feedbacks.type，请只改 0004 之后的新迁移，不要动这里。
--
